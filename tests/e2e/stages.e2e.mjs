/**
 * In-browser check of STAGED FOCUS, over the shipped corpus and the real globe.
 *
 * What the product asked for, and what only a picture and a live store can show:
 *
 *   · a staged event shows a STAGE STRIP in focus mode, above the pill, with
 *     "Overview" first and one chip per stage in `at` order;
 *   · Overview draws everything, exactly as before stages existed;
 *   · clicking a stage filters the drawing to that stage's layers plus the
 ...
 *
 * Run:  node tests/e2e/stages.e2e.mjs
...
const tag = process.env.SHOT_TAG ?? 'stages'
...
/** Everything the staged map is showing, as numbers a screenshot cannot be diffed on. */
const stateOf = (page) =>
  page.evaluate(() => ({
    focus: window.__events.focus?.itemId,
    stage: window.__events.stageId ?? null,
    stages: window.__events.focusStages.map((s) => s.id),
    ...
    chips: [...document.querySelectorAll('[data-test="stage-chip"]')].map((b) => ({
      stage: b.dataset.stage,
      text: b.textContent.trim(),
      on: b.classList.contains('on'),
    })),
    page: document.querySelector('[data-test="stage-page"] h3')?.textContent ?? null,
  }))
...
await check('the strip appears, Overview first, one chip per stage in at order', () => {
  eq(
    overview.chips.map((c) => c.stage),
...
await check('the overview is the whole plan — every layer, no stage', () => {
  ok(overview.stage === null, `arrived on stage ${overview.stage}`)
...
await page.click('[data-stage="kiev"]')
await settle(page, 2600)
const kiev = await stateOf(page)
await shot(page, '02-stage-kiev')
console.log(`    layers: ${kiev.layers.join(', ')}`)
console.log(`    page heading: ${kiev.page}`)
await check('the drawing filters to the stage plus the timeless layers', () => {
  ok(kiev.stage === 'kiev', `stage is ${kiev.stage}`)
  ok(
  ...
  // …and the December front, which belongs to the last stage, is gone
...
await check('the stage page shows, headed with the stage name', () => {
  ok(kiev.page?.includes('Kiev'), `stage page heading: ${kiev.page}`)
})
await check('the chip is the selected one', () => {
  eq(kiev.chips.filter((c) => c.on).map((c) => c.stage), ['kiev'], 'selected chips')
})
...
console.log('\n(c) another stage — "The counteroffensive (December)"')
await page.click('[data-stage="counteroffensive"]')
await settle(page, 2000)
const dec = await stateOf(page)
await shot(page, '03-stage-december')
...
await check('per-stage annotations do not leak between stages', () => {
...
await check('opens on its overview with four stages and its minor parts pinned', () => {
  eq(dday.stages, ['six-june', 'beachhead', 'cherbourg', 'breakout'], 'stages')
  ok(dday.stage === null, `arrived on stage ${dday.stage}`)
  for (const id of ['pointe-du-hoc', 'villers-bocage', 'cherbourg-1944'])
    ok(dday.visible.includes(id), `${id} is not on the globe`)
})
await page.click('[data-stage="cherbourg"]')
await settle(page, 2600)
const cherbourg = await stateOf(page)
await shot(page, '06-dday-cherbourg')
await check('a stage with a camera flies there, and filters to its own annotations', () => {
  ok(cherbourg.stage === 'cherbourg', `stage is ${cherbourg.stage}`)
...
/* ------------------------------------------------- a hand on the globe --- */
console.log('\n(f) a gesture during a stage flight')
await page.evaluate(() => window.__events.dismiss())
await settle(page, 900)
await page.evaluate(() => window.__events.showOnMap('barbarossa'))
await settle(page, 2600)
// Step to a stage with a camera and grab the planet while it is still flying.
await page.click('[data-stage="typhoon"]')
...
const pov = await page.evaluate(() => {
  ...
  const cam = window.__events.focusStages.find((s) => s.id === 'typhoon').camera
  return { lat: p.lat, lng: p.lng, cam }
})
console.log(
  `    camera at ${pov.lat.toFixed(2)},${pov.lng.toFixed(2)}; the stage asked for ` +
...
await check('the drag wins: the flight is abandoned where the hand took over', () => {
  ...
  ok(off > 0.5, `the tween landed anyway (${off.toFixed(3)}° from the stage camera)`)
})
...
const strip = await phone.$eval('[data-test="stage-strip"]', (el) => {
  ...
  return {
    ...
    scrollable: el.querySelector('.stages').scrollWidth > el.querySelector('.stages').clientWidth,
  }
})
console.log(
...
await phone.click('[data-stage="typhoon"]')
await settle(phone, 2400)
await shot(phone, '08-phone-stage-page')
const phoneStage = await stateOf(phone)
await check('a stage page opens over the map on a phone too', () => {
  ok(phoneStage.stage === 'typhoon', `stage is ${phoneStage.stage}`)
  ok(phoneStage.page?.includes('Typhoon'), `page heading: ${phoneStage.page}`)
})
await check('the strip is not buried under the open stage page', async () => {
  const hit = await phone.evaluate(() => {
    const strip = document.querySelector('[data-test="stage-strip"]')
...
await phone.click('[data-test="stage-back"]')
...
await check('the page’s own back control returns to the whole article', async () => {
  ...
  ok(s.stage === null, `still on ${s.stage}`)
  ok(s.page === null, 'the stage page is still up')
})
